...
def test_http_client_mutation(qlient_http_client):
    response = qlient_http_client.mutation.addBook(["title", "author"], title="1984", author="George Orwell")
    assert isinstance(response, GraphQLResponse)
...
def test_async_client_subscription(qlient_http_client):
    count = 0
    response = qlient_http_client.subscription.count(target=3, _subscription_id="123", _options={"foo": "bar"})
