...
class Client:
    ...
    def __init__(
        ...
        self._schema: Schema | None = schema if schema is not None else None

        self._query_service: QueryServiceProxy | None = None
        self._mutation_service: MutationServiceProxy | None = None
        self._subscription_service: SubscriptionServiceProxy | None = None
    ...
    def query(self) -> QueryServiceProxy:
        ...
        if self._query_service is None:
            schema = self.schema
            self._query_service = QueryServiceProxy(self.backend, self.settings, schema, self.plugins)
    ...
    def mutation(self) -> MutationServiceProxy:
        ...
        if self._mutation_service is None:
            schema = self.schema
            self._mutation_service = MutationServiceProxy(self.backend, self.settings, schema, self.plugins)
    ...
    def subscription(self) -> SubscriptionServiceProxy:
        ...
        if self._subscription_service is None:
            schema = self.schema
            self._subscription_service = SubscriptionServiceProxy(self.backend, self.settings, schema, self.plugins)
...
class AsyncClient(Client):
    ...
    def schema(self) -> Schema:
        ...
        """
        if self._schema is None:
            raise OutOfAsyncContext("Did you use 'async with AsyncClient() as client:'?")
        return self._schema
    ...
    def query(self) -> AsyncQueryServiceProxy:
        ...
        if self._query_service is None:
            schema = self.schema
            self._query_service = AsyncQueryServiceProxy(self.backend, self.settings, schema, self.plugins)
        return self._query_service
    ...
    def mutation(self) -> AsyncMutationServiceProxy:
        ...
        if self._mutation_service is None:
            schema = self.schema
            self._mutation_service = AsyncMutationServiceProxy(self.backend, self.settings, schema, self.plugins)
        return self._mutation_service
