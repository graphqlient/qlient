...
from qlient.core.models import Directive, Field, Fields, PreparedDirective
...
def test_graphql_subscription_request(graphql_subscription_request):
    ...
    assert graphql_subscription_request.query == "query testOperation { testOperation { foo bar } }"
...
def test_graphql_response(graphql_response):
    assert graphql_response.request.query == "query testOperation { testOperation { foo bar } }"
