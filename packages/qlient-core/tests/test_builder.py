...
def test_query_nesting_fields():
    ...
    actual = GQLQueryBuilder().action("hero").fields("name friends { name }").operation("query").build()
    assert expected == actual
...
def test_query_input():
    ...
    actual = (
        GQLQueryBuilder().fields("name height").action("human", variables={"id": '"1000"'}).operation("query").build()
    )
    assert expected == actual
...
def test_query_with_variables():
    ...
    actual = (
        ...
        .operation("query", name="HeroNameAndFriends", variables={"$episode": "Episode"})
        .build()
    )
    assert expected == actual
...
def test_remove_duplicate_spaces():
    expected = "{ query { hero { name } } }"
    actual = GQLQueryBuilder().remove_duplicate_spaces(" {  query  {  hero  {  name  }  }  } ")
    assert expected == actual
