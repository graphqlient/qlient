...
from qlient.core import AsyncBackend, GraphQLResponse
...
@pytest.mark.asyncio
async def test_backend_execute_subscription_iterator(async_strawberry_backend, monkeypatch):
    messages = [{"foo": "bar"}] * 10
...
@pytest.mark.asyncio
async def test_backend_execute_subscription_generator(async_strawberry_backend, monkeypatch):
    messages = [{"foo": "bar"}] * 10
